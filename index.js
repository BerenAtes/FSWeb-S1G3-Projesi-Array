...
function sonCesitiKaldir(sonCesit) {
  sonCesit.pop();
...
function ismeGoreCesitCikar(tatlar, lezzetAdı) {
  let Tarçın = tatlar.indexOf(lezzetAdı);
  let Tat = tatlar.splice(Tarçın, 1);
  return tatlar;
}
...
function ismeGoreFiltrele(tatlar, filtreDegeri) {
  ...
  for (let i = 0; i < tatlar.length; i++) {
    if (tatlar[i].includes(filtreDegeri)) {
      filtreliTatlar.push(tatlar[i]);
    }
  }
  return filtreliTatlar;
}
